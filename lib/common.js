...
const getEmailTemplate = (result, articles) => {
    const config = getConfig();

    const template = fs.readFileSync(path.join(__dirname, '../public/templates/article.html'), 'utf8');
    const layout = fs.readFileSync(path.join(__dirname, '../public/templates/articles.html'), 'utf8');
    ...
    */ 
    const articlesHtml = articles.map(e => mustache.render(template, {title: e.title, price: e.price, description: e.description}))
